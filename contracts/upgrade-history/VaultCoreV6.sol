// Changes: 1. _harvestReward() now uses at most rewardLiquidationThreshold CRV
//  		   for rebase; the rest goes to a specified address;
// 			2. In _harvestInterest(), interestLiquidationThreshold is now the
//   		   upper bound of interest earned, instead of low bound;
//   		3. Slightly optimized rebase() (necessary since contract size
 ...
 */
contract VaultCoreV6 is Initializable, OwnableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IVaultCore {
	...
	function rebase() external whenRebaseAllowed nonReentrant {
		...
		uint USDsNewSupply = USDsOldSupply;
		if (USDsIncrement > 0) {
			USDsNewSupply = USDsOldSupply.add(USDsIncrement);
        ...
        if (rwdTokenAddr != address(0)) {
			...
			if (ICurveGauge(crvGaugeAddress).claimable_reward(address(strategy), crvToken) > 0) {
				...
				if (rwdAmt > maxRebasingRwd) {
					address rwdReserve = 0x6d5240f086637fb408c7F727010A10cf57D51B62;
	...
	/**
	 * @notice harvest interesed earned in strategies
	 * @dev interestLiquidationThreshold is the maximum interest allowed;
	 *		if insterest earned is higher than that, txn will be reverted. Lower
