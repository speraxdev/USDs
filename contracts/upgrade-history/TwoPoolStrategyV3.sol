...
contract TwoPoolStrategyV3 is InitializableAbstractStrategyV2 {
     ...
     * @dev lp token should be redeemed to 'maxReturn' of  'returnAsset' token
     * @dev for max return
        ...
        returns (uint256 maxReturn, address returnAsset)
    {
        ...
        uint256 balanceNoSwap_original = 0;
        uint256 balanceSwap_other = 0;
        uint256 balanceSwap_original = 0;
        // redemption is not accurate when the amount of lp is too small
        if (totalPTokens > lpAssetThreshold) {
            // returned amt (in original token) when redeeming to original token
            balanceNoSwap_original = curvePool.calc_withdraw_one_coin(
                totalPTokens,
                int128(supportedAssetIndex)
            );
            // returned amt (in the other token) when redeeming to the other
            // token
            balanceSwap_other = curvePool.calc_withdraw_one_coin(
                totalPTokens,
                int128(index_other)
            );
            // returned amt (in the original token) when redeeming to the other
            // token
            balanceSwap_original = _convertBetween(
                index_other,
                supportedAssetIndex,
                balanceSwap_other
            );
        }
        // comparing returned amt between 1. redeeming to original token and
        // 2. redeeming to the other token
        if (balanceNoSwap_original > balanceSwap_original) {
            maxReturn = balanceNoSwap_original;
            returnAsset = assetsMapped[supportedAssetIndex];
        } else {
            maxReturn = balanceSwap_other;
            returnAsset = assetsMapped[index_other];
        }
    }
}
