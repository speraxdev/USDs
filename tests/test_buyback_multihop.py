...
@pytest.fixture(scope="module")
def usdt_token():
    return Contract.from_abi("ERC20", USDT, abi)


@pytest.fixture(scope="module")
def buyback_multihop_usdt_spa_weth(user_account): 
...
@pytest.fixture(scope="module")
def buyback_multihop_usdt_sol_weth(user_account): 
    return BuybackMultihop.deploy(
        SWAP_ROUTER,
        WETH,
        USDT, 
...
@pytest.fixture(scope="module")
def buyback_multihop_usdt_spa_weth_pf50(user_account): 
    return BuybackMultihop.deploy(
        SWAP_ROUTER,
        WETH,
        USDT, 
...
@pytest.fixture(scope="module")
def buyback_multihop_usdt_invalid_token_weth(user_account): 
    return BuybackMultihop.deploy(
        SWAP_ROUTER,
        WETH,
        USDT, 
...
def test_swap_successful(buyback_multihop_usdt_spa_weth, user_account, weth_token, usdt_token):
    balance1 = weth_token.balanceOf(user_account.address)

    allowance = usdt_token.allowance(user_account.address, buyback_multihop_usdt_spa_weth.address)
    if(allowance <= 0):
        usdt_token.approve(buyback_multihop_usdt_spa_weth.address, 10000000, {"from": user_account})

    buyback_multihop_usdt_spa_weth.swap(100000, {"from": user_account, "allow_revert": True, "gas_limit": 1000000})
...
def test_swap_with_no_pool(buyback_multihop_usdt_sol_weth, user_account, weth_token, usdt_token):
    failed = False
    allowance = usdt_token.allowance(user_account.address, buyback_multihop_usdt_sol_weth.address)
    if(allowance <= 0):
        usdt_token.approve(buyback_multihop_usdt_sol_weth.address, 10000000, {"from": user_account})
...
def test_swap_insufficient_balance(buyback_multihop_usdt_spa_weth, user_account_with_no_balance, weth_token, usdt_token):
    failed = False
    allowance = usdt_token.allowance(user_account_with_no_balance.address, buyback_multihop_usdt_spa_weth.address)
    try:
        if(allowance <= 0):
            usdt_token.approve(buyback_multihop_usdt_spa_weth.address, 1000, {"from": user_account_with_no_balance})

        with reverts("Insufficient funds"):
            buyback_multihop_usdt_spa_weth.swap(100000, {"from": user_account_with_no_balance, "allow_revert": True, "gas_limit": 1000000})
        failed = True
    except Exception:
       failed = False
    assert failed == False
...
def test_swap_invalid_pool_fee(buyback_multihop_usdt_spa_weth_pf50, user_account, weth_token, usdt_token):
    failed = False
    allowance = usdt_token.allowance(user_account.address, buyback_multihop_usdt_spa_weth_pf50.address)
    if(allowance <= 0):
        usdt_token.approve(buyback_multihop_usdt_spa_weth_pf50.address, 10000000, {"from": user_account})
...
def test_swap_invalid_token(buyback_multihop_usdt_invalid_token_weth, user_account, weth_token, usdt_token):
    failed = False
    allowance = usdt_token.allowance(user_account.address, buyback_multihop_usdt_invalid_token_weth.address)
    if(allowance <= 0):
        usdt_token.approve(buyback_multihop_usdt_invalid_token_weth.address, 10000000, {"from": user_account})
