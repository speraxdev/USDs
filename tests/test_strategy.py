...
def user(accounts):
    return accounts[9]
def test_collect_interest_pTokens(sperax, weth, accounts,owner_l2):
    (
        spa,
        usds_proxy,
        vault_core_tools,
...
def test_check_balance(sperax, usdt, invalid_collateral):
>>>>>>> dev
    (
        spa,
        usds_proxy,
...
=======
         balance = strategy_proxy.checkBalance(invalid_collateral, {'from': vault_proxy.address})
>>>>>>> dev
